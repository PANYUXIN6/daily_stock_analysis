...
- [改进] 选股与单股分析统一为量价波段：移除 PE/PB 过滤、估值评分和风险扣分，停止财报/分红质量查询，移除成长质量与预期重估技能；净利润断层改为不依赖财报的跳空涨停策略，保留旧历史记录。


- [改进] 将 Tushare 行情、股票索引、选股快照及净利润断层数据调用迁移到麦蕊，统一股/手/元与静态 PE 口径，组合财务报表、标准预告和等比复权行情；配置改用 MAIRUI_LICENCE，保留既有数据源 fallback。
- [文档] 增加麦蕊接口映射、真实请求结果及无法等价替代的字段与策略影响清单；同步 Web、部署和中英文配置指南。
- [测试] 增加麦蕊单位转换、证书脱敏、限流、财报/预告组合与缺失数据回归验证。

- [改进] 移除筹码分布数据获取、Agent 工具与配置入口；缩量回踩和底部放量取消筹码确认条件，新报告和数据质量评分不再依赖筹码指标，旧报告继续兼容读取。


- [改进] 移除稳健价值、双低选股、蓝筹收益质量、低波质量四个内置选股策略，Web/API 默认改用均衡多因子；同步清理专属测试断言并更新通用测试样例，保留全部 15 个单股分析策略与共享回测能力。

- [修复] DeepSeek 用量统计识别旧版裸模型名称 `deepseek`，避免遗漏历史调用。

- [改进] 用量统计仅纳入 DeepSeek，统一汇总、模型分组和最近调用范围，清理其他厂商专属统计用例。

- [改进] AI 模型接入收敛为 DeepSeek 官方云端 API，删除其他厂商配置与适配，保留同厂商备用模型、多 Key、Agent 和图片导入。

- [改进] 模型接入统一为云端 API，移除 Ollama、Hermes 本地 HTTP、本地免 Key 推理、Codex / Claude Code / OpenCode CLI 及 Codex App Server 问股，清理对应配置、状态接口、Web 入口和测试；保留云模型路由、模型回退、工具调用与诊断脱敏。

- [改进] 新闻与搜索增强仅保留 Bocha 和 Tavily，删除其他搜索提供方、设置、依赖及专属测试；Anspire 与 MiniMax 大模型接入不受影响。

- [改进] 移除持仓管理及账户、交易流水、CSV/Futu 导入、估值、回撤、止损预警、持仓 Agent、持仓筛选与关联 API；保留历史数据库记录，旧持仓告警不再执行。
...
- [修复] 修复 A 股瘦身后调度上下文变量与筛选模块导入遗漏，恢复机器人中文股票名称解析、登记指数点分前缀、历史代码规范化与持仓点分别名筛选，并阻止聊天分词将境外代码截取为 A 股。

- [chore] 客户端收敛为 Web：删除 Electron 桌面封装、自动更新、专属截图接口与桌面打包发布链路，保留 Web 配置备份和 PNG 分享。

- [chore] 删除 A 股复盘入口不可达的多市场分支、社交情绪空占位与海外数据源专属测试，将保留的行情补齐、新闻证据和指数测试迁移为 A 股用例。
- [改进] 将运行时、数据源、市场策略、代码解析、Web 市场选项与文档收敛为仅支持 A 股，并移除海外市场依赖与赞助推广信息。
