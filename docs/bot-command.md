...
- 股票：A 股 6 位数字（`600519`）。股票名称（`贵州茅台`）复用既有名称解析器（`resolve_name_to_code`）解析后提交。
...
- 未登记 CSI（如 `930956.CSI`）、不支持的代码形态（如 `12345`、未登记 `sh999999`）或无法识别的名称：返回明确错误，不提交任务。
...
- `/status` 显示的 LLM 可用性遵循系统统一运行时优先级：
  ...
  - `LLM_CHANNELS`
  - legacy provider 键（`DEEPSEEK_API_KEY`）
...
- 诊断增强不进行 silent migration：不会主动清空/删除 `LITELLM_*` 的历史值，仅在可用性诊断上提示。
