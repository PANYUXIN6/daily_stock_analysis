...
def _make_df(dates_and_closes):
    rows = [
        {"code": "600519", "date": d, "open": c, "high": c, "low": c, "close": c, "volume": 100, "amount": 0, "pct_chg": 0}
...
class AugmentRealtimeMarketDateTestCase(unittest.TestCase):
    ...
    def test_appends_virtual_row_with_market_local_date(
        ...
        """The virtual row uses the Shanghai market date."""
        # Use the exchange date supplied by the market clock.
        cn_market_now = datetime(2026, 3, 27, 10, 0)
        mock_now.return_value = cn_market_now
    ...
    def test_updates_existing_row_when_data_matches_market_date(
        ...
        mock_now.return_value = datetime(2026, 3, 27, 14, 0)
