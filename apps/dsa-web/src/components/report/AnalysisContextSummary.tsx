...
const MISSING_REASON_LABELS: Record<ReportLanguage, Record<string, string>> = {
  zh: {
    ...
    fundamental_context_missing: '资金与板块未进入本次分析，结论未使用资金与板块数据；请检查资金与板块数据源、网络或限流后重新分析',
    fundamental_pipeline_failed: '资金与板块抓取失败，本次分析未使用资金与板块数据；请检查数据源配置、网络或限流后重新分析',
    fundamentals_not_supported: '当前市场或标的不支持资金与板块数据，本次分析未使用该数据；请结合其他指标判断',
    fundamental_coverage_missing: '资金与板块覆盖数据未进入本次分析，结论可能缺少部分交易信息；请检查数据源覆盖范围后重新分析',
    fundamental_source_chain_missing: '未记录资金与板块来源链元数据；资金与板块是否进入本次分析以当前状态为准，请结合来源和告警复核数据出处',
