...
const fieldDescriptionMap: Record<string, string> = {
  ...
  TICKFLOW_PRIORITY: '控制 TickFlow 在普通 A 股日 K 回退链中的尝试顺序；已登记指数使用 Tencent → AkShare → TickFlow 固定链，不读取本配置；实时行情顺序由 REALTIME_SOURCE_PRIORITY 决定。',
  ...
  LITELLM_MODEL: '主模型，格式 provider/model（如 deepseek/deepseek-flash）。配置渠道后自动推断。',
  AGENT_LITELLM_MODEL: '默认模型配置用于问股的模型。留空时继承主模型；无 provider 前缀时按 deepseek/<model> 解析。',
  ...
  DEEPSEEK_API_KEY: 'DeepSeek 官方 API 密钥。填写后自动使用 deepseek-flash 模型。',
  ...
  MARKET_REVIEW_REGION: '大盘复盘市场区域，A 股版本固定为 cn。',
