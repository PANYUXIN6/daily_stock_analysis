...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.ai_model.LITELLM_MODEL': {
    ...
    usage: '推荐使用 provider/model 格式，例如 deepseek/deepseek-v4-flash、deepseek/deepseek-flash。',
    valueNotes: [
  ...
  'settings.ai_model.LLM_CHANNELS': {
    ...
    usage: '填写逗号分隔的渠道名，例如 deepseek,backup；每个渠道再配置 LLM_<NAME>_BASE_URL、LLM_<NAME>_API_KEY(S)、LLM_<NAME>_MODELS 等字段。',
    valueNotes: [
  ...
  'settings.ai_model.AGENT_LITELLM_MODEL': {
    ...
    usage: '使用 provider/model 格式。留空时继承普通分析主模型；裸模型名会按兼容逻辑归一到 deepseek/<model>。',
    valueNotes: [
      '适合给 Agent 使用推理能力更强或上下文更长的模型。',
      '该字段只影响 Agent 链路，不会改变普通个股分析的主模型。',
    ],
    impact: ['影响默认模型问股、策略选择和相关工具调用。'],
  ...
  'settings.ai_model.LLM_PROMPT_CACHE_HINTS_ENABLED': {
    ...
    valueNotes: ['DeepSeek 缓存由服务端管理，不发送其他厂商缓存参数。'],
  ...
  'settings.ai_model.LLM_REQUEST_DELAY': {
    title: 'DeepSeek 请求间隔', summary: '分析请求之间的等待秒数。', usage: '默认 2 秒，限流时可增加。', valueNotes: [], impact: ['影响分析速度。'], notes: [],
  },
  'settings.ai_model.VISION_MODEL': {
    title: 'DeepSeek 图片模型', summary: '图片导入使用的模型。', usage: '使用 deepseek/deepseek-flash。', valueNotes: [], impact: ['影响图片识别。'], notes: [],
  },
  'settings.ai_model.provider_keys': {
    title: '模型服务 API Key',
    summary: '配置DeepSeek 官方服务的访问密钥。',
  ...
  'settings.data_source.search_api_keys': {
    title: '搜索服务 API Key',
    summary: '配置 Bocha 或 Tavily 密钥，用于新闻检索与搜索增强。',
    usage: '多 Key 字段使用英文逗号分隔；系统优先使用 Bocha，再回退到 Tavily。',
  ...
  'settings.agent.AGENT_MAX_STEPS': {
    title: 'Agent 最大推理步数',
    summary: '控制 Agent 的推理步数上限。',
    usage: '使用默认模型时，默认值让每个子 Agent 使用各自预设步数；调高后统一提升，调低后裁剪预设步数。',
    valueNotes: [
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.ai_model.LITELLM_MODEL': {
    ...
    usage: 'Use provider/model format, such as deepseek/deepseek-v4-flash, deepseek/deepseek-flash.',
    valueNotes: [
  ...
  'settings.ai_model.LLM_CHANNELS': {
    ...
    usage: 'Use comma-separated names such as deepseek,backup; then configure LLM_<NAME>_BASE_URL, LLM_<NAME>_API_KEY(S), and LLM_<NAME>_MODELS for each channel.',
    valueNotes: [
  ...
  'settings.ai_model.AGENT_LITELLM_MODEL': {
    ...
    impact: ['Affects default-model Agent chat, strategy selection, and Agent tool calls.'],
  ...
  'settings.ai_model.LLM_PROMPT_CACHE_HINTS_ENABLED': {
    ...
    valueNotes: ['DeepSeek manages caching on the server.'],
  ...
  'settings.ai_model.LLM_REQUEST_DELAY': {
    title: 'DeepSeek request delay', summary: 'Seconds between analysis requests.', usage: 'Defaults to 2 seconds; increase if rate limited.', valueNotes: [], impact: ['Affects analysis speed.'], notes: [],
  },
  'settings.ai_model.VISION_MODEL': {
    title: 'DeepSeek vision model', summary: 'Model used for image import.', usage: 'Use deepseek/deepseek-flash.', valueNotes: [], impact: ['Affects image recognition.'], notes: [],
  },
  'settings.ai_model.provider_keys': {
  ...
  'settings.llm_channel.protocol': {
    ...
    usage: 'Only the official DeepSeek API is supported.',
  ...
  'settings.llm_channel.api_surface': {
    title: 'API Surface',
    summary: 'Uses the Chat Completions API.',
    usage: 'Keep the Chat Completions default.',
    ...
    notes: ['Only DeepSeek Chat Completions is supported.'],
  },
  'settings.llm_channel.base_url': {
    title: 'Base URL',
    summary: 'Endpoint root for this channel.',
    usage: 'Use https://api.deepseek.com, optionally with /v1.',
  ...
  'settings.llm_channel.api_key': {
    ...
    valueNotes: ['Enter the API key provided by your cloud service.'],
  ...
  'settings.agent.AGENT_MAX_STEPS': {
    ...
    summary: 'Caps Agent reasoning steps.',
    usage: 'With the default-model Agent, the default lets each sub-agent keep its preset; raising the value lifts all sub-agents, while lowering it caps larger presets.',
    valueNotes: [
    ...
    impact: ['Affects Agent reasoning depth, duration and token consumption.'],
  ...
  'settings.agent.context_compression': {
    ...
    notes: [
      ...
      'When enabled, the Agent primary model compresses longer chat history.',
    ],
  },
  'settings.agent.event_monitor': {
    title: 'Event Monitor',
