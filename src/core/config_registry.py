...
SCHEMA_VERSION = "2026-09-21-deepseek-only"
...
_FIELD_DEFINITIONS: Dict[str, Dict[str, Any]] = {
    ...
    "LITELLM_MODEL": {
        "title": "Primary Model",
        "description": "Primary model in provider/model format (e.g. deepseek/deepseek-flash, deepseek/deepseek-flash, deepseek/deepseek-flash). If empty, it is auto-inferred from available API keys or channel declarations.",
        "category": "ai_model",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "AGENT_LITELLM_MODEL": {
        ...
        "description": "Optional Agent-only primary model in provider/model format. When empty, Agent inherits the primary model. Bare model names are normalized to deepseek/<model>.",
        "category": "ai_model",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "LITELLM_FALLBACK_MODELS": {
        ...
        "description": "Comma-separated fallback models tried when the primary model fails (e.g. deepseek/deepseek-flash,deepseek/deepseek-flash). Only DeepSeek models are supported.",
        "category": "ai_model",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    # ------------------------------------------------------------------
    # AI Model – DeepSeek official
    # ------------------------------------------------------------------
    "LLM_REQUEST_DELAY": {
        "title": "Request Delay", "description": "Delay between DeepSeek analysis requests in seconds.",
        "category": "ai_model", "data_type": "number", "ui_control": "number",
        "is_sensitive": False, "is_required": False, "is_editable": True,
        "default_value": "2", "options": [], "validation": {"min": 0}, "display_order": 8,
        "help_key": "settings.ai_model.LLM_REQUEST_DELAY", "examples": ["LLM_REQUEST_DELAY=2"], "docs": [{"label": "DeepSeek", "href": "https://api-docs.deepseek.com/"}], "warning_codes": [],
    },
    "VISION_MODEL": {
        "title": "Vision Model", "description": "DeepSeek model supporting image input.",
        "category": "ai_model", "data_type": "string", "ui_control": "text",
        "is_sensitive": False, "is_required": False, "is_editable": True,
        "default_value": "deepseek/deepseek-flash", "options": [], "validation": {}, "display_order": 9,
        "help_key": "settings.ai_model.VISION_MODEL", "examples": ["VISION_MODEL=deepseek/deepseek-flash"], "docs": [{"label": "DeepSeek", "href": "https://api-docs.deepseek.com/"}], "warning_codes": [],
    },
    "DEEPSEEK_API_KEY": {
        "title": "DeepSeek API Key",
        "description": "Official DeepSeek API key (from https://platform.deepseek.com). Defaults to deepseek/deepseek-flash. Multiple keys and DeepSeek channels are supported.",
        "category": "ai_model",
        "data_type": "string",
        "ui_control": "password",
        "is_sensitive": True,
    ...
    "AGENT_MAX_STEPS": {
        ...
        "description": f"Maximum reasoning-step limit for the Agent. At the default ({AGENT_MAX_STEPS_DEFAULT}), each default-model sub-agent keeps its own preset. When raised above {AGENT_MAX_STEPS_DEFAULT}, all sub-agents adopt this value. When lowered below a sub-agent's preset, that sub-agent is capped at this value.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_ORCHESTRATOR_TIMEOUT_S": {
        "title": "Agent Timeout",
        "description": "Shared timeout budget in seconds for Agent execution. Single-agent runs use it as the overall ReAct loop budget; multi-agent mode uses it as the cooperative pipeline budget. Set to 0 to disable the overall timeout.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
...
def _infer_category(key: str) -> str:
    ...
    if key.startswith("BACKTEST_"):
        return "backtest"
    if key.startswith(("LITELLM_", "DEEPSEEK_", "LLM_")):
        return "ai_model"
    ...
    ) or key in ("ENABLE_REALTIME_QUOTE",):
        return "data_source"
    if key.startswith((
        "WECHAT",
        "FEISHU",
