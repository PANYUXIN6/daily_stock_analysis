...
MARKET_LIGHT_REGIONS = frozenset({"cn"})
MARKET_LIGHT_ALERT_REGIONS = MARKET_LIGHT_REGIONS
...
def normalize_market_region(region: str) -> str:
    ...
    if value not in MARKET_LIGHT_REGIONS:
        raise ValueError(f"market target must be cn: {region}")
    return value
...
def normalize_market_alert_region(region: str) -> str:
    ...
    if value not in MARKET_LIGHT_ALERT_REGIONS:
        raise ValueError(f"market alert target must be cn: {region}")
    return value
