...
def load_strategy(filepath: Path) -> Strategy:
    ...
    screening = ScreeningConfig(
        gap_limit_up=(GapLimitUpConfig(**_optional_mapping(
            screening_data, "gap_limit_up", filepath,
            allowed_keys=set(GapLimitUpConfig.__dataclass_fields__),
        )) if "gap_limit_up" in screening_data else None),
...
def list_strategies(strategies_dir: Path | None = None) -> list[StrategyInfo]:
    ...
    for s in strategies.values():
        daily_required = requires_daily_features(s.screening.hard_filters) or s.screening.gap_limit_up is not None
        infos.append(StrategyInfo(
            ...
            required_daily_fields=_required_daily_fields(s.screening.hard_filters, s.screening.gap_limit_up),
            active_filters=_active_hard_filters(s.screening.hard_filters, s.screening.gap_limit_up),
...
def compare_strategies(
    ...
    base_daily = requires_daily_features(base.screening.hard_filters) or base.screening.gap_limit_up is not None
    target_daily = requires_daily_features(target.screening.hard_filters) or target.screening.gap_limit_up is not None
    differences = {
        ...
        "required_daily_fields": _list_diff(
            _required_daily_fields(base.screening.hard_filters, base.screening.gap_limit_up),
            _required_daily_fields(target.screening.hard_filters, target.screening.gap_limit_up),
        ),
        "active_filters": _list_diff(
            _active_hard_filters(base.screening.hard_filters, base.screening.gap_limit_up),
            _active_hard_filters(target.screening.hard_filters, target.screening.gap_limit_up),
        ),
        ...
        "gap_limit_up_values": _mapping_diff(
            asdict(base.screening.gap_limit_up) if base.screening.gap_limit_up else {},
            asdict(target.screening.gap_limit_up) if target.screening.gap_limit_up else {},
        ),
...
def _strategy_data_requirements(strategy: Strategy, *, daily_required: bool) -> list[str]:
    ...
    if strategy.screening.gap_limit_up:
        requirements.extend(["limit_prices"])
    return requirements
...
def _strategy_compare_summary(strategy: Strategy, *, daily_required: bool) -> dict[str, object]:
    return {
        ...
        "requires_daily_features": daily_required,
        "active_filters": _active_hard_filters(strategy.screening.hard_filters, strategy.screening.gap_limit_up),
...
def _active_hard_filters(filters_config: HardFilterConfig, gap_limit_up: GapLimitUpConfig | None = None) -> list[str]:
    active: list[str] = []
    ...
    for item in fields(HardFilterConfig):
        ...
        if value != default and value is not None and value is not False:
            active.append(name)
    if gap_limit_up:
        active.extend(["gap_limit_up", "gap_unfilled"])
    return active
...
def _required_daily_fields(filters_config: HardFilterConfig, gap_limit_up: GapLimitUpConfig | None = None) -> list[str]:
    checks = [
    ...
    if gap_limit_up:
        required.extend(["trade_date", "open", "high", "low", "close", "vol", "adjusted_open", "adjusted_high", "adjusted_low", "adjusted_close", "up_limit"])
    return required


def _strategy_profile_keys(screening: ScreeningConfig) -> dict[str, list[str]]:
    profile_values = {
        "gap_limit_up": asdict(screening.gap_limit_up) if screening.gap_limit_up else {},
