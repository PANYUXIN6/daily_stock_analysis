...
DEEPSEEK_MODEL_DEFAULT = "deepseek/deepseek-flash"
DEEPSEEK_API_BASE = "https://api.deepseek.com"
_MANAGED_LITELLM_KEY_PROVIDERS = {"deepseek"}
SUPPORTED_LLM_CHANNEL_PROTOCOLS = ("deepseek",)
SUPPORTED_LLM_CHANNEL_API_SURFACES = ("chat_completions",)
...
def apply_litellm_api_surface(model: str, api_surface: Optional[str]) -> str:
    if not is_supported_llm_channel_api_surface_value(api_surface):
        raise ValueError("仅支持 DeepSeek Chat Completions")
    return normalize_llm_channel_model(model, "deepseek")


def resolve_llm_channel_protocol(
    protocol: Optional[str], *, base_url: Optional[str] = None,
    models: Optional[List[str]] = None, channel_name: Optional[str] = None,
) -> str:
    explicit = canonicalize_llm_channel_protocol(protocol)
    if explicit and explicit != "deepseek":
        return ""
    if base_url and base_url.rstrip("/") not in {DEEPSEEK_API_BASE, DEEPSEEK_API_BASE + "/v1"}:
        return ""
    return "deepseek"
...
def normalize_llm_channel_model(model: str, protocol: Optional[str], base_url: Optional[str] = None) -> str:
    ...
    if not normalized:
        return ""
    if not resolve_llm_channel_protocol(protocol, base_url=base_url):
        raise ValueError("仅支持 DeepSeek 官方 API")
    if normalized.startswith("deepseek/"):
        normalized = normalized.split("/", 1)[1]
    if not normalized.startswith("deepseek-") or "/" in normalized:
        raise ValueError("仅支持 DeepSeek 模型")
    return f"deepseek/{normalized}"


def find_incompatible_llm_channel_models(
    models: List[str], protocol: Optional[str], api_surface: Optional[str] = None,
    base_url: Optional[str] = None,
) -> List[str]:
    invalid = []
    for model in models:
        try:
            normalize_llm_channel_model(model, protocol, base_url)
        except ValueError:
...
def resolve_unified_llm_temperature(model: str) -> float:
    return parse_env_float(os.getenv("LLM_TEMPERATURE"), 0.7, field_name="LLM_TEMPERATURE")
...
def _matches_route_set(model: str, routes: set[str]) -> bool:
    """Loose safety match for route identity checks, not normal route availability."""
...
def normalize_agent_litellm_model(model: str, configured_models: Optional[set[str]] = None) -> str:
    normalized = (model or "").strip()
    if configured_models and normalized in configured_models:
        return normalized
    return normalize_llm_channel_model(normalized, "deepseek")
...
class Config:
    ...
    # === AI 分析配置 ===
    # LiteLLM unified model config (provider/model format, e.g. deepseek/deepseek-flash)
    ...
    vision_model: str = DEEPSEEK_MODEL_DEFAULT
    ...
    # 实时行情数据源优先级（逗号分隔）
    # 推荐顺序：tencent > akshare_sina > efinance > akshare_em > mairui
    # - tencent: 腾讯财经，有量比/换手率/市盈率等，单股查询稳定（推荐）
    # - akshare_sina: 新浪财经，基本行情稳定，但无量比
    # - efinance/akshare_em: 东财全量接口，数据最全但容易被封
    # - mairui: 麦蕊，需有效 licence，接口权限依证书版本
    ...
    def __post_init__(self) -> None:
        for name in ("litellm_model", "agent_litellm_model", "vision_model"):
            value = getattr(self, name)
            if value:
                setattr(self, name, normalize_agent_litellm_model(value, set(get_configured_llm_models(self.llm_model_list))))
        self.litellm_fallback_models = [
            normalize_agent_litellm_model(model, set(get_configured_llm_models(self.llm_model_list))) for model in self.litellm_fallback_models
        ]
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        if not llm_model_list and not llm_blocks_legacy_fallback:
            llm_model_list = cls._deepseek_keys_to_model_list(deepseek_api_keys)
            if not litellm_model and deepseek_api_keys:
                litellm_model = DEEPSEEK_MODEL_DEFAULT
        ...
        return cls(
            ...
            vision_model=os.getenv('VISION_MODEL', DEEPSEEK_MODEL_DEFAULT).strip(),
    ...
    def _parse_litellm_yaml(cls, config_path: str) -> List[Dict[str, Any]]:
        ...
        for entry in model_list:
            params = entry.get("litellm_params", {})
            try:
                params["model"] = normalize_llm_channel_model(
                    params.get("model", ""), "deepseek", params.get("api_base")
                )
                if not params["model"] or params.get("custom_llm_provider") not in (None, "deepseek"):
                    return []
            except ValueError:
                _logger.warning("LITELLM_CONFIG only supports DeepSeek official models")
                return []
    ...
    def _parse_llm_channels(cls, channels_str: str) -> List[Dict[str, Any]]:
        ...
        channels, _issues, _blocks = cls._parse_llm_channels_with_issues(channels_str)
        return channels
    ...
    def _parse_llm_channels_with_issues(
        ...
        channels: List[Dict[str, Any]] = []
        issues: List[ChannelConfigIssue] = []
        ...
        for raw_name in channels_str.split(','):
            ...
            if not is_supported_llm_channel_api_surface_value(api_surface_raw):
                issues.append(ChannelConfigIssue(
            ...
            if not protocol:
                issues.append(ChannelConfigIssue(f"LLM_{ch_upper}_PROTOCOL", "unsupported_provider", "仅支持 DeepSeek 官方 API"))
                continue
            ...
            if incompatible_models:
                issues.append(ChannelConfigIssue(
                    ...
                    "unsupported_model",
                    (
                        "Only DeepSeek official models are supported; "
        ...
        return channels, issues, blocks_legacy_fallback
    ...
    @classmethod
    def _deepseek_keys_to_model_list(cls, deepseek_keys: List[str]) -> List[Dict[str, Any]]:
        return [
            {"model_name": "__legacy_deepseek__", "litellm_params": {
                "model": "__legacy_deepseek__", "api_key": key,
            }} for key in deepseek_keys if key and len(key) >= 8
        ]
    ...
    def _resolve_realtime_source_priority(cls) -> str:
        """
        Resolve realtime source priority with automatic mairui injection.

        When MAIRUI_LICENCE is configured but REALTIME_SOURCE_PRIORITY is not
        explicitly set, automatically prepend 'mairui' to the default priority
        ...
        mairui_licence = os.getenv('MAIRUI_LICENCE', '').strip()
        if mairui_licence:
            # Token configured but no explicit priority override
            # Prepend mairui so the paid source is tried first
            import logging
            logger = logging.getLogger(__name__)
            resolved = f'mairui,{default_priority}'
            logger.info(
                f"MAIRUI_LICENCE detected, auto-injecting mairui into realtime priority: {resolved}"
            )
            return resolved
    ...
    def has_search_capability_enabled(self) -> bool:
        """Whether Bocha or Tavily search is configured."""
        return bool(self.bocha_api_keys or self.tavily_api_keys)

    def is_agent_available(self) -> bool:
        """Explicitly enabled Agent mode or an inferred cloud model enables Chat."""
    ...
    def validate_structured(self) -> List[ConfigIssue]:
        ...
        if not self.mairui_licence:
            issues.append(ConfigIssue(
                severity="info",
                message="未配置麦蕊证书，将使用其他数据源",
                field="MAIRUI_LICENCE",
            ))
        ...
        if not self.llm_model_list and not has_direct_env_model:
            if self.litellm_config_path:
                ...
                issues.append(ConfigIssue(
                    severity="error",
                    message=(
                        "未配置 DeepSeek API。请配置 DEEPSEEK_API_KEY。"
            ...
            issues.append(ConfigIssue(
                ...
                message=(
                    ...
                    "建议尽早配置主模型（格式如 deepseek/deepseek-flash）"
                ),
                field="LITELLM_MODEL",
            ))
        ...
        def _has_runtime_source_for_model(model: str) -> bool:
            if not model or _uses_direct_env_provider(model):
                return True
            return model.startswith("deepseek/") and any(
                k and len(k) >= 8 for k in self.deepseek_api_keys
            )
        ...
        if not self.has_search_capability_enabled():
            issues.append(ConfigIssue(
                severity="info",
                message="未配置搜索引擎能力 (Bocha/Tavily)，新闻搜索功能将不可用",
        ...
        if self.vision_model and not self.llm_model_list and not any(len(k) >= 8 for k in self.deepseek_api_keys):
            issues.append(ConfigIssue(severity="warning", message="图片识别缺少可用的 DeepSeek API Key", field="VISION_MODEL"))
...
def get_api_keys_for_model(model: str, config: Config) -> List[str]:
    if not model.startswith("deepseek/"):
        return []
    return [key for key in config.deepseek_api_keys if key and len(key) >= 8]


def extra_litellm_params(model: str, config: Config) -> Dict[str, Any]:
    return {"api_base": DEEPSEEK_API_BASE}
